...
func (config *Config) Login(writer http.ResponseWriter, request *http.Request) {
	...
	userJWTToken, errJWTToken := MakeJWT(userData.ID, config.Secret, time.Second*3600*6)
	if errJWTToken != nil {
	...
	writer.WriteHeader(http.StatusOK)
	log.Printf("User %s logged in with success. Redirecting to Home Page...", userData.Name)
}
